...
const LINK = "text-[#1a4f8b] underline underline-offset-2 hover:text-[#0f3a6b]";
const MAX_TILT = 8;
...
const Star = ({ fill, className }: { fill: string; className: string }) => (
...
/* ---------- main component ---------- */
export default function BookCardComponentDetail(props: BookDetailType) {
      ...
      <div className="grid gap-6 md:grid-cols-[250px_1fr]">
        ...
        <aside className="h-fit rounded-md border border-gray-200 p-3">
          ...
          <div className="mt-4 flex gap-0.5 overflow-hidden rounded-md">
            ...
            <button className={`w-10 text-white ${BLUE}`} aria-label="More reading options">
              <Icon d={ICONS.chevron} className="mx-auto h-4 w-4" />
            </button>
          </div>
          ...
          <button className="mt-3 flex w-full items-center justify-center gap-1.5 rounded-md border border-gray-200 bg-white py-2 text-sm text-gray-700 shadow-sm hover:bg-gray-50">
            Buy <Icon d={ICONS.chevron} className="h-3.5 w-3.5" />
          </button>

          {/* Add to List (split) */}
          <div className="mt-3 flex overflow-hidden rounded-md border border-gray-300 bg-[#f4f1e8] text-gray-800">
            <button className="flex-1 py-2.5 text-[15px] hover:bg-[#ece8da]">Add to Library</button>
            <button className="w-10 border-l border-gray-300 hover:bg-[#ece8da]" aria-label="List options">
              <Icon d={ICONS.chevron} className="mx-auto h-4 w-4" />
            </button>
          </div>

          {/* Rate this book */}
          <div className="mt-4 flex justify-center gap-2" aria-label="Rate this book">
            {[1, 2, 3, 4, 5].map((n) => (
              <button key={n} className="text-gray-300 hover:text-amber-500" aria-label={`${n} stars`}>
          ...
          <div className="mt-4 flex justify-around border-t border-[#e3d9bd] pt-4 text-xs text-gray-700">
            {[
              ...
              <button key={label} className="flex flex-col items-center gap-1 hover:text-[#1769b0]">
          ...
          <h3 className="mt-5 text-sm font-bold text-gray-900">Download Options</h3>
        </aside>
        ...
        <div className="min-w-0">
          ...
          <nav className="flex flex-wrap items-center gap-1 border-y border-gray-200 py-2 text-[13px] font-bold text-gray-800">
            <a className="rounded-full bg-[#1769b0] px-4 py-2 text-white" href="#">Overview</a>
            <a className="rounded-full px-4 py-2 hover:bg-gray-100" href="#">
              View {props.editionCount ?? 0} Editions
            </a>
            {["Details", "Reviews", "Lists", "Related Books"].map((t) => (
              <a key={t} className="rounded-full px-4 py-2 hover:bg-gray-100" href="#">{t}</a>
            ))}
          </nav>
            ...
            <div>
              {props.editionOf && (
                <p className="font-serif italic text-gray-500">
                  An edition of{" "}
                  <a href="#" className="underline">{props.editionOf.title}</a>{" "}
                  {props.editionOf.year && (
                    <span className="text-sm not-italic">({props.editionOf.year})</span>
                  )}
                </p>
              )}
              <h1 className="font-serif text-4xl text-gray-900">{props.title}</h1>
              <p className="mt-3 font-serif text-lg text-gray-600">
                by <a href="#" className={LINK}>{props.author}</a>
              </p>
            </div>
            ...
            <div className="flex items-start gap-3 text-right text-xs text-gray-600">
              {props.lastEditedBy && (
                <p className="leading-relaxed">
                  Last edited by <a href="#" className={LINK}>{props.lastEditedBy}</a>
                  <br />
                  {props.lastEditedDate} | <a href="#" className={LINK}>History</a>
                </p>
              )}
          ...
          <div className="mt-5 flex flex-wrap items-center gap-x-2 gap-y-1 text-[15px] text-gray-700">
            <span className="flex" aria-label={`${props.rating} out of 5`}>
              {[1, 2, 3, 4, 5].map((n) => (
                <Star key={n} className="h-5 w-5" fill={n <= Math.round(rating) ? "#e0a422" : "#d8d8d8"} />
              ))}
            </span>
            <span>{rating.toFixed(1)} ({props.ratingCount ?? 0} ratings)</span>
            <span>·</span>
            <span>{props.wantToRead ?? 0} Want to read</span>
            <span>·</span>
            <span>{props.haveRead ?? 0} Have read</span>
          </div>

          {/* Description with fade + Read More */}
          {props.description && (
            <div className="mt-8">
              <div className={`relative overflow-hidden leading-7 text-gray-700 ${expanded ? "" : "max-h-[6.5rem]"}`}>
          ...
          <div className="mt-9 grid gap-4 sm:grid-cols-3">
            {[
              ...
              <div key={label as string} className="rounded-md border border-gray-200 px-4 py-4 text-center">
                <p className="text-sm text-gray-500">{label}</p>
                <p className={`mt-1 ${isLink ? "text-[#1a4f8b]" : "text-gray-900"}`}>{value ?? "—"}</p>
              </div>
            ))}
          </div>
            ...
            <p className="mt-6 text-[15px] text-gray-500">
              ...
              {props.previewLanguages.map((l, i) => (
                <React.Fragment key={l}>
                  <a href="#" className={LINK}>{l}</a>
                  {i < props.previewLanguages!.length - 1 && " "}
                </React.Fragment>
              ))}
            </p>
          ) : null}
          ...
          <div className="mt-4 space-y-4">
            <ChipRow label="Subjects" items={props.subjects} />
            <ChipRow label="People" items={props.people} />
          </div>
        </div>
      </div>
    </div>
